...
public sealed class BroadcastHubUIRoot : UIRoot<BroadcastHubUIRoot.Refs>
{
    ...
    [SerializeField] private DmThreadPanel dmThreadPanel;
    ...
    private void BindHandlers()
    {
        ...
        if (pollDetailPanel)
        {
            pollDetailPanel.OnConfirmRequested -= HandlePollPanelConfirm;
            pollDetailPanel.OnConfirmRequested += HandlePollPanelConfirm;
        }
    }
    ...
    public void ShowTab(TabKind tab)
    {
        ...
        if (pollDetailPanel) pollDetailPanel.Show(tab == TabKind.Poll);
        if (dmThreadPanel) dmThreadPanel.Show(tab == TabKind.Dm);
    }
    ...
    public void SetPollOptions(params string[] options)
    {
        ...
        int count = options != null? 
            options.Length 
            : 0;

        for (int i = 0; i < _pollOptionBtns.Length; i++)
        {
            Button btn = _pollOptionBtns[i];
            TMP_Text label = _pollOptionLabels[i];
        ...
        // 우측 패널 confirm도 잠금
        if (pollDetailPanel)
            pollDetailPanel.SetConfirmInteractable(false);
    }

    private void SetPollSelected(int index)
    {
        if (!_valid)
            return;
        ...
        if (pollDetailPanel)
            pollDetailPanel.SetConfirmInteractable(_selectedPollIndex >= 0);
    }
